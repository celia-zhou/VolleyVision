...
export const CoachSetsForm = () => {
  ...
  useEffect(() => {
    ...
    console.log(matchId);
    let setsPath = `users/${playerId}/matches/${matchId}/stats`;
    let oppPath = `users/${playerId}/matches`;
    ...
    getDoc(doc(db, oppPath, matchId)).then((snapshot) => {
      const data = snapshot.data();
  ...
  return (
    <div>
      ...
      <form onSubmit={addSet}>
        ...
        <span> </span>
        <br></br>
        <span style={{ position: "absolute", right: "17px", bottom: "2px" }}>
